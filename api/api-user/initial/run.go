...
import (
	...
	"hy-im/api/api-user/handler"
	"hy-im/api/api-user/service"
	"os"
	"os/signal"
	"syscall"
)
...
func Run(f string) {
	...
	// 链路追踪
	io, err := hy_tracer.InitTracer(imName.RpcApiUser, c.Trace.Addr)
	if err != nil {
	...
	if err := innerPt.RegisterApiUserHandler(srv.Server(), &userHandler); err != nil {
