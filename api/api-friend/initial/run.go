...
import (
	...
	"hy-im/api/api-friend/handler"
	"hy-im/api/api-friend/service"
	"os"
	"os/signal"
	"syscall"
)
...
func Run(f string) {
	...
	// 链路追踪
	io, err := hy_tracer.InitTracer(imName.RpcApiFriend, c.Trace.Addr)
	if err != nil {
	...
	friendHandler := handler.Handler{}
	if err := innerPt.RegisterApiUserHandler(srv.Server(), &friendHandler); err != nil {
